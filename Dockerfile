...
RUN export NODE_ENV=production; yarn install --frozen-lockfile --no-optional --production && yarn cache clean --force && mkdir ./static && yarn prisma generate && chown node:node -R .
USER node
ENTRYPOINT [ "node" ]
CMD [ "dist/main" ]
